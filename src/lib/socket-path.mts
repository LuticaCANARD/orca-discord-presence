...
function pushBase(bases: string[], value: string | undefined): void {
 ...
 */
export function posixRuntimeBases({
  env = {},
  uid
}: {
  env?: NodeJS.ProcessEnv
 ...
 */
export function probeSocketPaths({
  platform,
  env = {},
  uid,
  exists = existsSync
}: SocketProbeOptions & { exists?: (path: string) => boolean }): string[] {
