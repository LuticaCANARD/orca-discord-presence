...
export type PluginCapabilityKind = 'workspace:read' | 'terminal:send' | 'notifications:show' | 'storage' | 'secrets' | 'events:subscribe' | 'settings:own';
/** `AGENT_STATUS_STATES`. */
...
export type HostCallMap = {
    ...
    'events.subscribe': {
        ...
        result: {
            subscribed: PluginEventName[];
        };
    };
};
export type HostMethod = keyof HostCallMap;
export interface OrcaHostApi {
    call<M extends HostMethod>(method: M, ...params: HostCallMap[M]['params'] extends void ? [] : [params: HostCallMap[M]['params']]): Promise<HostCallMap[M]['result']>;
}
...
export interface OrcaEventsApi {
    on<E extends PluginEventName>(event: E, handler: (payload: PluginEventMap[E]) => unknown | Promise<unknown>): void;
}
